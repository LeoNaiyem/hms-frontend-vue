  ...
  <div class="m-3 dark-mode-support">
    ...
    <div class="card p-4 table-responsive">
      ...
      <div class="d-flex justify-content-end gap-2 mt-4">
        <router-link :to="`/appointments/${appt.id}/edit`" class="btn btn-warning">
          Edit
        </router-link>
...
const fetchAppointment = async () => {
  try {
    ...
    if (res.data.success) {
      appt.value = res.data.data;
    } else {
      showToastMessage("Failed to load appointment.", "bg-danger");
    }
  } catch (err) {
    console.error("Failed to fetch appointment:", err);
    showToastMessage("Could not load appointment data.", "bg-danger");
  }
};
...
const deleteAppointment = async () => {
  try {
    const res = await axios.delete(`${BASE_URL}/appointments/${appt.value.id}`);
    if (res.data.success) {
      showToastMessage("Appointment deleted successfully!", "bg-success");
