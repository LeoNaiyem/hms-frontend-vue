  ...
  <div class="m-3 dark-mode-support">
    ...
    <div class="card mb-3">
      <div class="d-flex justify-content-between align-items-center p-3">
        ...
        <router-link to="/departments/create" class="btn btn-success">
          Add New Department
        </router-link>
      </div>
    </div>
    ...
    >
      <div style="flex-grow: 2">
        <input
          ...
          @input="fetchDepartments(1)"
        />
      </div>
      <div class="flex-grow-1">
        <button class="btn btn-success w-100" @click="fetchDepartments(1)">
          Apply
        </button>
      </div>
      <div class="flex-grow-1">
    ...
    <nav class="mt-3" v-if="totalPages > 1">
      <ul class="pagination justify-content-center">
        <li class="page-item" :class="{ disabled: currentPage === 1 }">
          <button
            class="page-link"
            @click="fetchDepartments(currentPage - 1)"
        ...
        >
          <button class="page-link" @click="fetchDepartments(page)">
            {{ page }}
          </button>
        </li>
        ...
        <div class="modal-dialog">
          <div class="modal-content p-4 rounded gap-3 d-flex bg-light">
            ...
            <div class="modal-body">
              Delete department
              <strong>#{{ departmentToDelete?.id }}</strong> —
...
async function fetchDepartments(page = 1) {
  ...
  try {
    ...
    console.error(err);
    showToastMessage("Failed to fetch departments.", "bg-danger");
  } finally {
    loading.value = false;
  }
}
...
async function deleteDepartment() {
  ...
  try {
    const res = await axios.delete(`${BASE_URL}/departments/${id}`);
    if (res.data.success) {
      showToastMessage("Department deleted.", "bg-success");
      fetchDepartments(currentPage.value);
    } else {
      showToastMessage("Failed to delete department.", "bg-danger");
    }
  } catch (err) {
    console.error(err);
...
/* Dark mode toast color overrides */
