  ...
  <div class="m-3 dark-mode-support">
    ...
    >
      ...
      <div class="flex-grow-1">
        <input
          ...
          type="date"
          class="form-control"
          @change="fetchPatients(1)"
        />
      </div>
      <div class="flex-grow-1">
        <button class="btn btn-success w-100" @click="fetchPatients(1)">
          Apply
        </button>
      </div>
      <div class="flex-grow-1">
    ...
    <nav class="mt-3" v-if="totalPages > 1">
      <ul class="pagination justify-content-center">
        ...
        >
          <button class="page-link" @click="fetchPatients(page)">
            {{ page }}
          </button>
        </li>
        ...
        <div class="modal-dialog">
          <div class="modal-content p-4 rounded gap-3 d-flex bg-light">
            ...
            <div class="modal-body">
              Delete patient <strong>#{{ patientToDelete?.id }}</strong> - <strong>{{ patientToDelete?.name }}</strong>?
            </div>
...
async function fetchPatients(page = 1) {
  ...
  try {
    const params = {
      ...
      ...(filters.value.dob && { dob: filters.value.dob }),
    };
    const res = await axios.get(`${BASE_URL}/patients`, { params });
    if (res.data.success) {
    ...
    console.error(err);
    showToastMessage("Failed to fetch patients.", "bg-danger");
  } finally {
    loading.value = false;
  }
}
...
async function deletePatient() {
  const id = patientToDelete.value?.id;
  if (!id) return;

  try {
    const res = await axios.delete(`${BASE_URL}/patients/${id}`);
    if (res.data.success) {
      showToastMessage("Patient deleted.", "bg-success");
