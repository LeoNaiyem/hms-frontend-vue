  ...
  <div class="m-3 dark-mode-support">
    ...
    <div class="card p-4 table-responsive">
      <table class="table table-bordered mb-0">
        <tbody>
          ...
          <tr>
            <th>Ward</th>
            <td>
              <div>{{ bed.ward?.name || "N/A" }}</div>
              <small>{{ bed.ward?.ward_code || "-" }}</small>
            </td>
          </tr>
          <tr>
            <th>Bed Type</th>
          ...
          <tr>
            <th>Created At</th>
            <td>{{ formatDateTime(bed.created_at) }}</td>
          </tr>
          <tr>
            <th>Updated At</th>
            <td>{{ formatDateTime(bed.updated_at) }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="d-flex justify-content-end gap-2 mt-4">
        <router-link :to="`/beds/${bed.id}/edit`" class="btn btn-warning">
          Edit
        </router-link>
...
const fetchBed = async () => {
  try {
    ...
    if (res.data.success) {
      ...
      showToastMessage("Failed to load bed data.", "bg-danger");
    }
  } catch (err) {
    console.error("Failed to fetch bed:", err);
    showToastMessage("Could not load bed data.", "bg-danger");
  }
};
...
const deleteBed = async () => {
  try {
    const res = await axios.delete(`${BASE_URL}/beds/${bed.value.id}`);
    if (res.data.success) {
      showToastMessage("Bed deleted successfully!", "bg-success");
...
<!-- Non-scoped styles for transitions and modals -->
<style>
.modal-backdrop {
  position: fixed;
  top: 0;
