        ...
        <div class="row ps-head">
          <div class="col-9 d-flex gap-4">
            ...
            <div style="min-width: 345px" class="doctor-section pe-5 border-end border-info-subtle">
              ...
              <p class="text-uppercase fw-bold">[Health care specialized hospital]</p>
            </div>
            ...
            <div class="patient-section">
              ...
              <div class="my-3 d-flex align-items-center gap-4">
                ...
                <input v-model="form.age" class="form-control p-1" type="number" placeholder="Age" />
              </div>
              ...
              <div class="mb-3 d-flex align-items-center gap-4">
                ...
                <div class="d-flex">
                  <label class="form-check d-flex align-items-center gap-1">
                    <input v-model="form.sex" class="form-check-input" type="radio" value="0" /> Male
                  </label>
                  <label class="form-check d-flex align-items-center gap-1 ms-1">
                    <input v-model="form.sex" class="form-check-input" type="radio" value="1" /> Female
                  </label>
                  <label class="form-check d-flex align-items-center gap-1 ms-1">
                    <input v-model="form.sex" class="form-check-input" type="radio" value="2" /> Other
                  </label>
                </div>
              </div>
            </div>
        ...
        <div class="d-flex justify-content-between ps-main my-5 px-5">
          ...
          <div class="w-75 border-end border-info-subtle pe-4">
            ...
            <div class="d-flex my-4 align-items-center gap-2">
              <div>
                ...
                <select v-model="medicineForm.medicine_id" class="form-select py-1 px-2">
              ...
              <div>
                <label>Dose</label>
                <select v-model="medicineForm.dose" class="form-select py-1 px-2">
                  <option value="">Select</option>
                  <option v-for="dose in doseOptions" :key="dose" :value="dose">{{ dose }}</option>
                </select>
              </div>

              <div>
                <label>Course</label>
                <select v-model="medicineForm.days" style="min-width: 100px;" class="form-select py-1 px-2">
                  <option value="">Select</option>
              ...
              <div>
                ...
                <select v-model="medicineForm.suggestion" style="min-width: 165px;" class="form-select py-1 px-2">
                  <option value="">Select</option>
                  <option v-for="sug in suggestionOptions" :key="sug" :value="sug">{{ sug }}</option>
                </select>
              </div>

              <div class="mt-4">
                <button @click="addMedicine" class="btn btn-info py-1 px-2">Add</button>
              </div>
            </div>
              ...
              <tbody>
                <tr v-for="(item, index) in items" :key="index">
                  ...
                  <td class="text-center">
                    <button @click="removeMedicine(index)" class="btn px-2 btn-sm btn-danger">
          ...
          <div class="w-25 ps-3">
            ...
            <div class="d-flex gap-2 align-items-center my-2">
              ...
              <button @click="addDiagnosis" class="btn mt-0 py-1 px-2 btn-info">Add</button>
            </div>

            <ol class="list-group list-group-numbered mt-1 mb-3">
              <li v-for="(diag, index) in diagnosisItems" :key="index" class="list-group-item d-flex justify-content-between align-items-center p-2">
                {{ diag }}
                <span style="cursor:pointer;height:20px;width:20px"
            ...
            <div class="mb-3">
              ...
              <textarea v-model="form.cc" class="p-2 form-control" rows="2" placeholder="e.g., Fever, cough..."></textarea>
            </div>

            <div class="mb-3">
              <label class="form-label">Relevant Findings (RF):</label>
              <textarea v-model="form.rf" class="p-2 form-control" rows="2" placeholder="e.g., Smoker, diabetic..."></textarea>
            </div>

            <div class="mb-3">
              <label class="form-label">Doctor's Advice:</label>
              <textarea v-model="form.advice" class="p-2 form-control" rows="2" placeholder="e.g., Take rest, drink fluids..."></textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="ps-bottom position-relative">
        <div class="d-flex column-gap-5">
          <div class="ps-timestamp">
            <p><i class="fa-solid fa-calendar-days"></i>&nbsp; <span class="fw-light"> Days</span>: Mon, Tue, Wed, Thu, Fri</p>
            <p><i class="fa-solid fa-clock"></i>&nbsp; <span class="fw-light"> Timings</span>: 05:00 PM - 08:30 PM</p>
          </div>
        ...
        <button @click="savePrescription" class="btn py-1 px-2 btn-lg btn-info position-absolute top-25 start-50">
...
function addMedicine() {
  const med = medicines.value.find(m => m.id === medicineForm.value.medicine_id)
  if (!med) return
